...
int main(){
   ...
   rep(i,0,n-1) cin>>pages[i];
//   vector<vector<int>> dp(n,vector<int>(tot+1,-1));
//   function<int(int,int)>dfs=[&](int index,int tot){
//       if(tot==0 || index==n) return 0;
//       if(dp[index][tot]!=-1) return dp[index][tot];
//       int maxvalue=INT_MIN;
//       if(tot>=cost[index]){
//           maxvalue=max(maxvalue,pages[index]+dfs(index+1,tot-cost[index]));
//       }
//       maxvalue=max(maxvalue,dfs(index+1,tot));
//       return dp[index][tot]=maxvalue;
//   };
//   cout<<dfs(0,tot)<<"\n";
    ...
    for(int i=1;i<=n;i++){
        for(int c=1;c<=tot;c++){
            if(c>=cost[i-1]){
                dp[i][c]=max(dp[i-1][c],pages[i-1]+dp[i-1][c-cost[i-1]]);
            }else{
                dp[i][c]=dp[i-1][c];
            }
        }
