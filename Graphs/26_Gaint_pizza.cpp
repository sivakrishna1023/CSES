...
// Kosaraju’s Algorithm application, in other word's an 2-SAT algo 
...
int main(){
    ...
    cin>>n>>m;
    int totalnodes=2*m;
    StrongDAG dag(totalnodes);
    auto complement=[&](int node){
    ...
    rep(i,1,n){
        auto addwish=[&](){
            ...
            int number;
            cin>>number;
            return (c =='+')?number:number+m;
        };
        int a=addwish();
        int b=addwish();
        dag.add_weight(complement(a),b);
        dag.add_weight(complement(b),a);
    }
    dag.process();
