...
int main(){
   ...
   auto checkconnection=[&](){
       ...
       while(!q.empty()){
           ...
           q.pop();
           for(int i:G[top]){
               int node=x[i]+y[i]-top;
               if(visit[node]==0){
                   // don't avoid here else you get TLE (common sence)
                   visit[node]=1;
                   q.push(node);
               }
           }
...
// One way to approach but TLE

// int main(){
//    lld n,m;
//    cin>>n>>m;
//    vector<vector<lld>> graph(n+1,vector<lld>(n+1,0));
//    vector<lld> degrees(n+1,0);
//    while(m--){
//        lld a,b;
//        cin>>a>>b;
