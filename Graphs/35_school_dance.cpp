...
int main(){
   ...
   int tot=n+m+2;
   int boy_start=0,girl_start=n;
   Graph_flow gf(tot);
   while(k--){
       int u,v;
       cin>>u>>v;
       gf.add_edge(boy_start+u,girl_start+v,1ll);
   }
   repI(i,1,n) gf.add_edge(source,boy_start+i,1ll);
   repI(i,1,m) gf.add_edge(girl_start+i,sink,1ll);
   lld ans=gf.maxflow(source,sink);
   cout<<ans<<"\n";
   for(auto [from,to,cap,used]:gf.e){
       if(used!=1 || source==from || sink==to) continue;
       cout<<(from-boy_start)<<" "<<(to-girl_start)<<"\n";
   }
   return 0;
}